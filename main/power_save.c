...
static void wifi_power_save(void) {
	...
	wifi_config_t wifi_config = { .sta = { .ssid = DEFAULT_SSID, .password =
			DEFAULT_PWD, .listen_interval = DEFAULT_LISTEN_INTERVAL, }, };
...
void app_main() {
	...
	if (storage_init == ESP_ERR_NVS_NO_FREE_PAGES
			|| storage_init == ESP_ERR_NVS_NEW_VERSION_FOUND) {
	...
	esp_pm_config_esp32_t pm_config = { .max_freq_mhz =
			CONFIG_EXAMPLE_MAX_CPU_FREQ_MHZ, .min_freq_mhz =
			CONFIG_EXAMPLE_MIN_CPU_FREQ_MHZ,
